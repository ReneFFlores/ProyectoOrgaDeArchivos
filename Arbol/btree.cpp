...
/*Arbol-B del proyecto, creado originalmente en consolapara solo probarlo, eventualmente importado como un reescrito en QT*/
...
int Tree::BusquedaBinaria(vector<Index> vector,unsigned long key){
    int mid;
    int low = 0;
...
int Tree::EliminarBusquedaBinaria(vector<Index> vector,unsigned long key){
    int mid;
    int low = 0;
    int high = vector.size() -1;
    while(low <= high){
        ...
        if(currentNode->getSons().size() == 0){// si es una hoja
            ...
            if(currentNode->getKeys().size() !=0 ){// si tiene elementos
                if(currentNode->getKeys().size() < orden-1){// que el numero de claves no sobrepasen el limite
                        int posicion = BusquedaBinaria(currentNode->getKeys(), key.getKeys());
                        if(posicion == -1){
            ...
            while('a'){
                //a hojanos movemos hasta el nodo donde deberia ir la clave (hasta la hoja)
                int posicion = BusquedaBinaria(currentNode->getKeys(), key.getKeys());
                if(posicion != -1){
...
int Tree::Promote(int page, int mitad){
    ...
    Index keyPromoted = arbol.at(page)->getKeys().at(mitad);
    vector<Index> tempKeys = currentNode->getKeys();
    int posicion = BusquedaBinaria(currentNode->getKeys(),keyPromoted.getKeys());
    vector<int> tempSons;
    if(posicion == -1){
...
void Tree::Eliminar(Index key){
    ...
    if(root != -1){
        ...
        int posicion = EliminarBusquedaBinaria(currentNode->getKeys(), key.getKeys());
        while (posicion == -1){//Recorrer arbol hasta localizar clave
            currentNode = arbol.at(cont);
            posicion = EliminarBusquedaBinaria(currentNode->getKeys(), key.getKeys());
            if(posicion != -1){
                break;
            }
            cont++;
        ...
        currentNode->setKeys(keys);
            ...
            while('a'){//nos movemos hasta el nodo donde deberia ir la clave (hasta la hoja)
                if(currentNode->getSons().size() != 0 ){
...
void Tree::Underflow(Node* currentNode){
    ...
    if(currentNode->getKeys().size() < minimoDeClavesAceptadas){// se verifica que esta en underflow
        ...
        if(currentNode->getKeys().size() > posicion){
            // si hay hermano a la derecha por el que esta underflow
            if(arbol.at(currentNode->getSons().at(posicion+1))->getKeys().size() > minimoDeClavesAceptadas){
                //se verifica el cmo esta el hermno derecho
                vector<Index> keys = arbol.at(page)->getKeys();
        ...
        }//FIN si hay hermano a la derecha
        //CASO CONCATENACION
        //si el hermano quedaria underflow si tuviera q dar (osea q no puede dar el hermano)
        entro = false;
        ...
        //no termnia en undeflow despues del cambio, verifica
        ...
        arbol.at( page)->setFather(-5);// para indicar que el nodo fue borrado
            ...
            //si la posicion se encuentra en 0
            int hermano = currentNode->getSons().at(posicion+1);
    ...
    }//ya no verifica si es un underflow o no
}
//FIN UNDERFLOW

//sobre el sucesor inmediato----------------------------------------------------------------------------------------------------
int Tree::PaginaSucesorInmediato(Node* currentNode,int posicion){
    currentNode = arbol.at(currentNode->getSons().at(posicion+1));
    while('a'){//nos memos hasta el nodo donde deberia ir la clave (hasta la hoja)
        if(currentNode->getSons().size() != 0 ){
            currentNode = arbol.at(currentNode->getSons().at(0));
        }else
            return currentNode->getPage();
...
int Tree::Buscar(unsigned long key){
    ...
    while('a'){//nos movemos hasta el nodo donde deberia ir la clave (hasta la hoja)
        int posicion = EliminarBusquedaBinaria(currentNode->getKeys(), key);
        if(posicion != -1){
            ...
            currentNode = arbol.at(currentNode->getSons().at(BusquedaBinaria(currentNode->getKeys(), key)));
        }
    }//while true
    return -1;
}
